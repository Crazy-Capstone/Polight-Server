...
| `POLICY_DOCUMENT_NOT_FOUND` | 404 | 약관 문서를 찾을 수 없습니다. | 존재하지 않거나 내 소유가 아닌 문서 |
| `EMPTY_POLICY_DOCUMENT_FILE` | 400 | 업로드할 약관 파일은 비어 있을 수 없습니다. | 0바이트 파일 업로드 |
| `POLICY_DOCUMENT_TOO_LARGE` | 413 | 업로드할 수 있는 파일 크기를 초과했습니다. | multipart 용량 초과 |
| `POLICY_DOCUMENT_STORAGE_FAILED` | 500 | 약관 파일을 저장하지 못했습니다. | 로컬/S3 저장 실패 |
...
| 2 | POST | `/api/v1/trips` | ✓ | 여행 세션 생성 |
| 3 | GET | `/api/v1/trips` | ✓ | 내 여행 목록 |
| 4 | GET | `/api/v1/trips/{tripId}` | ✓ | 여행 단건 조회 |
| 5 | PATCH | `/api/v1/trips/{tripId}` | ✓ | 여행 정보 수정 |
| 6 | POST | `/api/v1/trips/{tripId}/documents` | ✓ | 약관 파일 업로드 |
| 7 | GET | `/api/v1/trips/{tripId}/documents` | ✓ | 약관 목록 |
...
**201 Created** — 응답 헤더 `Location: /api/v1/trips/{id}`
...
**에러**: `INVALID_INPUT`(400), `INVALID_TRIP_PERIOD`(400), `AUTHENTICATION_REQUIRED`(401), `USER_NOT_FOUND`(404)
...
2) POST /api/auth/kakao/login          → accessToken 저장
   ↓
3) POST /api/v1/trips                  → tripId
   ↓
4) POST /api/v1/trips/{tripId}/documents  (multipart)  → documentId
   ↓
5) POST .../documents/{documentId}/analysis            → status: PROCESSING
   ↓
6) GET  .../documents/{documentId}/analysis  를 폴링   → COMPLETED / FAILED
```
