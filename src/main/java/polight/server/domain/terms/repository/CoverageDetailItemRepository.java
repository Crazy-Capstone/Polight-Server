...
public interface CoverageDetailItemRepository extends JpaRepository<CoverageDetailItem, UUID> {
  ...
  List<CoverageDetailItem> findByTermsCoverageIdOrderBySortOrderAsc(UUID termsCoverageId);

  /** 규칙별로 한 번씩 조회하면 규칙 수만큼 쿼리가 나간다. 화면에 필요한 것을 한 번에 가져온다. */
  List<CoverageDetailItem> findByTermsCoverageIdInOrderBySortOrderAsc(Collection<UUID> termsCoverageIds);
